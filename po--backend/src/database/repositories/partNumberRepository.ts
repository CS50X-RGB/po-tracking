...
class PartNumberRepository {
  ...
  public async createpartNumbers(
    partNumberObjects: PartNumberCreationInterface[],
  ) {
    try {
      ...
      const existingDescriptions = new Set(
        existingDocs.map((doc) => doc.description),
      );
      ...
      if (toInsert.length > 0) {
        inserted = await PartNumberModel.insertMany(toInsert, {
          ordered: false,
        });
      }
      return {
  ...
  public async getPartNumber(name: string) {
    try {
      const part = await PartNumberModel.findOne({ name }).lean();
      return part;
    } catch (error) {
  ...
  public async getPartNumbers(page: number, offset: number, search?: string) {
    try {
      ...
      const partNumber = await PartNumberModel.find(query)
        .skip(skip)
        .limit(offset)
        .lean();
  ...
  public async getPartNumberById(partNumber: any) {
    try {
      const partNumberEntity =
        await PartNumberModel.findById(partNumber).lean(); // ✅ chain lean() before await
